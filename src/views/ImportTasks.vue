      ...
      <aside class="task-sidebar">
        <div class="pane-header">
          ...
          <button
            ...
            class="header-action"
            type="button"
            @click="clearFinishedImportTasks"
          >
            清除已完成
          </button>
        </div>
...
import {
  ...
  importTasks,
  type ImportStepPreview,
  type ImportTask,
  type ImportTaskStatus,
  type ImportTaskStep,
} from '../services/importTasks'
...
const openedWriteStep = computed(() =>
  visibleSteps.value.find((step) => step.id === openedWriteStepId.value) || null
)
...
watch(
  () => [selectedId.value, visibleSteps.value.length, liveLine.value, selectedTask.value?.summary],
  async () => {
    await nextTick()
